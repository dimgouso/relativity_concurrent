...
function initGUI(){
	...
	lftSourcePos=gui.add(cntrls,"leftSourcePosition",1,20).step(1).name("Left Source :Position");
	lftSourcePos.onChange(function(newValue){
		source1X=source1Xcenter+(newValue-10)*20;
		setSource1Xpos();
    });

	rigtSourcePos=gui.add(cntrls,"rightSourcePosition",1,20).step(1).name("RIght Source :Position");
	rigtSourcePos.onChange(function(newValue){
		source2X=source2Xcenter+(newValue-10)*20;
		setSource2Xpos();
    });

	attenuation=gui.add(cntrls,"attenuation").listen().name("Wave attenuation");
	...
	noOfFrnts=gui.add(cntrls,"noOfFronts",1,maxNoOfFronts).step(1).name("Number of Wave Fronts");
	noOfFrnts.onChange(function(newValue){
		noOfFronts=newValue;
		reset();
    });
	
    frntColor=gui.addColor(cntrls,"frontsColor").name("Color of WaveFronts μετώπων");
